...
router.post("/", (request, response) => {
  if (
    ...
    const theQuery = "INSERT INTO DEMO(Name, Message) VALUES (?, ?);";
    const values = [request.body.name, request.body.message];

    pool.query(theQuery, values, (err, results, fields) => {
      if (err) {
...
router.get("/:name?", (request, response) => {
  const theQuery = "SELECT name, message FROM Demo WHERE name LIKE ?";
